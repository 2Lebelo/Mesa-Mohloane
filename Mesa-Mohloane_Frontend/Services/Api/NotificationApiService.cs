...
{
    Task<PagedResultDto<NotificationDto>?> GetMineAsync(int page = 1, int pageSize = 10, bool unreadOnly = false);
...
public sealed class NotificationApiService : ApiClientBase, INotificationApiService
{
    ...
    public NotificationApiService(IHttpClientFactory httpFactory, IHttpContextAccessor httpContextAccessor)
        ...
        int page = 1, int pageSize = 10, bool unreadOnly = false)
    {
        try
        {
            var query = $"?page={page}&pageSize={pageSize}&unreadOnly={unreadOnly.ToString().ToLowerInvariant()}";
    ...
    public async Task<int> GetUnreadCountAsync()
    {
        try
        {
            ...
            if (root.TryGetProperty("unreadCount", out var unreadCount)) return unreadCount.GetInt32();
            if (root.TryGetProperty("count", out var count)) return count.GetInt32();
            return 0;
        }
        catch { return 0; }
    }
    ...
    private static async Task<string?> ReadErrorAsync(HttpResponseMessage res, string? existingJson = null)
    {
        try
        {
